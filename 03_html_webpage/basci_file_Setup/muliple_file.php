...
if($_FILES['photo']){
    ...
    for($index=0; $index<$totalFiles; $index++){
        if(in_array($_FILES['photo']['type'][$index], $allowedTypes) && $_FILES['photo']['size'][$index] < 5*1024*1024){
            move_uploaded_file($_FILES['photo']['tmp_name'][$index], 'myfiles/'.$_FILES['photo']['name'][$index]);
        }
    }
